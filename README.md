<div align="center">

# ZenninMaki — Inka
**Análise e Desenvolvimento de Sistemas **

*Converse. Conecte. Inkamuri.*

[![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat&logo=dotnet&logoColor=white)]()
[![C#](https://img.shields.io/badge/C%23-239120?style=flat&logo=c-sharp&logoColor=white)]()
[![WPF](https://img.shields.io/badge/WPF-5C2D91?style=flat)]()
[![SignalR](https://img.shields.io/badge/SignalR-6B8299?style=flat)]()

</div>

### Sobre mim
Olá! Sou o Inka. Estudo ADS e gosto de aprender coisas nem tão novas assim — como **Ruby, Python, Rust e Lua**. Curto criar coisas leves que não atrapalhem o jogo e com cara boa.

### Em destaque
**[Inkamuri](https://github.com/ZenninMaki/inkamuri)** — chat comunitário com web ASP.NET Core + SignalR e cliente Windows WPF nativo (sem WebView2), tema `#140A0F` e perfis customizáveis.

### Stack que curto
`Ruby` • `Python` • `Rust` • `Lua` • `C#` • `ASP.NET Core` • `WPF` • `SQL Server`

### Atualmente estudando
Recomendo **TypeScript** pra você, Inka — combina perfeito com seu C#/.NET e te abre portas pro front moderno e pro Node. É leve, tipada e muito usada em projetos como o Inkamuri se um dia quiser um front em React.

### Projetos em destaque
**[Inkamuri](https://github.com/ZenninMaki/inkamuri)** — chat Global com modo visitante, loja InkCoins, Premium e cliente Windows WPF leve. Stack: `.NET 10 • SignalR • SQL Server`.

---
<div align="center">Feito com café e Inkamuri ☕</div>
